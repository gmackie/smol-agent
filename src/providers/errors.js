 ...
 * @param {string} provider - Provider name (for contextual messages)
 * @param {string} envVar - Environment variable name for API key
 * @returns {object} { message, actionable }
 */
export function formatAPIError(status, body, provider, envVar) {
  ...
  const errorType = errorDetails?.type || errorDetails?.error?.type || "";
