 ...
 */
async function ensureBankDir(cwd) {
 ...
 * @param {string} bankKey - One of: projectContext, techContext, progress, learnings
 * @returns {Promise<string|null>} File content or null if not found
 */
export async function readBankFile(cwd, bankKey) {
 ...
 */
export async function writeBankFile(cwd, bankKey, content) {
  const config = BANK_FILES[bankKey];
 ...
 *
 * @param {string} cwd - Project root
 * @returns {Promise<string|null>} Formatted memory bank content, or null if empty
 */
export async function loadMemoryBank(cwd) {
  const bankPath = path.join(cwd, MEMORY_BANK_DIR);
 ...
 */
export function getBankFileKeys() {
 ...
 */
export function getBankTemplate(bankKey) {
