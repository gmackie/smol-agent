 ...
 */
async function checkSkillResources(skillDir) {
  const result = { hasScripts: false, hasReferences: false, hasAssets: false };
  try {
 ...
 */
function createSkill(data, content, file, source, skillDir) {
  const name = data.name || path.basename(file, ".md") || path.basename(skillDir || file);
 ...
 */
async function loadStandardSkills(dirPath, source) {
  const skills = [];
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        logger.debug(`Could not read ${skillFile}: ${err.message}`);
      }
    }
  } catch {
    // dir doesn't exist
 ...
 */
async function loadLegacySkills(dirPath, source) {
  const skills = [];
  try {
    const entries = await fs.readdir(dirPath, { withFileTypes: true });
 ...
 */
export async function loadSkillResource(cwd, skillName, resourcePath) {
 ...
 */
export async function getSkillPath(cwd, skillName) {
 ...
 */
export async function getSkillNames(cwd) {
  const skills = await loadSkills(cwd);
