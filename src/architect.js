...
const ARCHITECT_SYSTEM_PROMPT = `You are an architect agent that analyzes codebases and produces implementation plans.
 ...
 * @returns {Promise<string>} The implementation plan
 */
export async function architectPass(client, model, task, options = {}) {
  ...
  for (let i = 0; i < MAX_ARCHITECT_ITERATIONS; i++) {
    ...
    try {
      ...
      logger.error(`Architect pass failed: ${err.message}`);
      return `(Architect failed: ${err.message})`;
    }
 ...
 */
export function formatPlanForEditor(plan, originalTask) {
